...
| `20_reset` | التصفير: يرفض بلا كلمة التأكيد وبيد الباريستا · يمسح الحركات ويُبقي الكتالوج · الحارس يرجع |
| `21_cups` | الكوب يتبع حرارة المشروب: ورق للساخن · بلاستك للبارد · لا كوب للجلوس · نفاد أحدهما لا يوقف الآخر |
| `22_count_sheet` | سند المتوقّع: الأكواب والبيع والهدر والشراء · يُصفَّر بكل عدّة · والملغاة خارجه |
| `23_retail` | البضاعة: فاتورة واحدة مع المشروب · التغليف للطلب لا للصنف · وعزل الربح والتكلفة |

وخارج `run-all.sh` (لأنه يحتاج node وقاعدةً ثانية):

```bash
db/tests/backup-roundtrip.sh   # هل تُستعاد النسخة الاحتياطية مطابقةً؟
```
...
النتائج المثبتة (١٢٣ حالة) في `docs/KHAZAF-POS-TECHSPEC.md` §19.
