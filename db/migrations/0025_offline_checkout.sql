-- =====================================================================
-- 0025 · البيع بلا إنترنت: الفاتورة تحمل وقتها الحقيقي
--
-- انقطاع الإنترنت في العراق ليس حالةً نادرة، وبيعٌ لا يتمّ لأن الشبكة
-- غابت خسارةٌ مضاعفة: زبونٌ ينتظر ومالٌ لا يُسجَّل. فالكاشير يُتمّ البيع
-- محلياً ويرفعه لاحقاً.
--
-- والخطر الحقيقي هنا ليس فقدان الفاتورة — المفتاح الفريد يمنع تكرارها
-- (§21) — بل **وقتها**. بيعٌ تمّ ١١ ليلاً ورُفع ٨ صباحاً كان سيُكتب بوقت
-- الرفع، فيقع في يوم محاسبي آخر ووردية أخرى: مبيعات ليلة أمس تنتقل إلى
-- اليوم، ونقدُه يظهر في درج وردية لم تقبضه. كل ما بُني في 0017 يسقط.
--
-- فأُضيف وسيط ثانٍ عشر `p_occurred_at`. فارغاً يعني «الآن» فلا يتغيّر شيء
-- للنداءات القائمة، ومملوءاً يعني «هذه لحظة البيع» فتُكتب في الطلب والدفعة
-- وحركة المخزون وحركة الكاش معاً — لا في بعضها دون بعض، وإلا تناقض الدفتر
-- مع الفاتورة.
--
-- والقاعدة العامة: **لا يُترك للقاعدة أن تفترض أن وقت الكتابة هو وقت
-- الحدث.** هما مختلفان كلّما تأخّرت الكتابة.
-- =====================================================================

-- النسخة ذات ١١ وسيطاً تُسقَط: الوسيط الثاني عشر له قيمة افتراضية، فلو
-- بقيت النسختان لصار النداء بـ١١ وسيطاً **غامضاً** ورفضته القاعدة —
-- «function checkout(...) is not unique». أي أن كل بيع كان سيتوقّف.
-- ونسخة الوسائط التسعة تبقى: عدد وسائطها مختلف فلا لبس.
drop function if exists checkout(uuid,uuid,uuid,uuid,text,text,integer,text,jsonb,uuid,jsonb);
...
comment on function checkout(uuid,uuid,uuid,uuid,text,text,integer,text,jsonb,uuid,jsonb,timestamptz) is
  'البيع. `p_occurred_at` لحظة البيع الحقيقية — تُمرَّر للفواتير التي تمّت بلا إنترنت ورُفعت لاحقاً.';
