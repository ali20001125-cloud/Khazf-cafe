...
مكافئ الجرعات    = |الفرق| ÷ material_dose(material_id)
```

**الجرعة تُشتقّ من الوصفة، لا تُكتب باليد** (هجرة ٠٠٢٢). `material_dose()` تجرّب
بالترتيب: تجاوزاً يدوياً في `materials.dose_grams` إن كُتب، ثم غرامات القهوة في
وصفات المشروبات التي تُقدَّم بهذا المحصول، ثم الكمية الأكثر شيوعاً في بنود
الوصفات (حليب ١٥٠مل · كوب ١ · غطاء ١)، ثم كمية الإضافة (سيروب ١٥مل)، ثم — لمادة
بديلة كحليب الشوفان — جرعة المادة التي تحلّ محلّها.

السبب: العمود أُضيف في ٠٠١١ وبقي فارغاً لكل مادة حقيقية، فبقي عمود «≈ جرعة»
فارغاً وتعطّل معه تنبيه `high_waste` بصمت (شرطه `dose_grams > 0`). ما يُطلب من
إنسانٍ أن يتذكّره سيُنسى؛ والوصفة تعرف الجرعة أصلاً.

**أربع قواعد لا تُكسَر:**

1. **النسبة عتبة تنبيه، لا كمية هدر مسموحة.** ٤٪ لا تعني أن ٤٪ «مقبولة».
2. **النسبة وحدها لا تقرّر.** نقصٌ يعادل **مشروبين فأكثر** أحمر مهما صغرت نسبته.
3. **الفرق لا يُكتب أبداً كـ`WASTE`.** يُسمّى `unexplained_variance` ويبقى فرقاً.
4. **لا يُسمّى «سرقة»** (§57). النظام يكتشف الفرق ولا يعرف سببه.

مثال حقيقي من محاكاة اليومين (`db/tests/11_two_days.sql`): سُرقت ثلاثة مشروبات
بلا تسجيل، فنقص **٥٤غ** حبوب — **٠٫٤١٪** فقط من مخزون ١٣ كغ، أي تحت أي عتبة
نسبة معقولة. لكنه **ثلاث جرعات كاملة**، فيظهر أحمر. بدون هذا الاختبار الثاني
يختبئ كوب كامل داخل نسبة صغيرة كلّما كبر المخزون (§34).

ولماذا مشروبان لا مشروب واحد؟ لأن مشروباً واحداً يقع بالخطأ فعلاً — صبّة زائدة،
ميزان يزيغ — ونظامٌ يصرخ كل يوم يُفقِد التنبيه معناه. ومن يسرق مشروباً واحداً كل
يوم لا يفلت: يلتقطه كشف النمط أدناه.
...
| 40 | **§34 مكافئ الجرعات** | فرق −18غ = **−0.38٪** (تحت العتبة) لكن **1.00 جرعة** · `label=unexplained_variance` | ✅ |
...
#### ح) الإلغاء والإرجاع والدرج (`db/tests/06` · `07`)

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 47 | إلغاء فاتورة مدفوعة | الطلب `VOIDED` ويبقى في التاريخ · **يخرج من مبيعات الكاش** | ✅ |
| 48 | إرجاع جزئي (1,000 من 3,000) | `PARTIALLY_REFUNDED` + حركة كاش −1,000 | ✅ |
| 49 | استكمال الإرجاع (2,000) | `REFUNDED` + حركة كاش −2,000 | ✅ |
| 50 | إرجاع يتجاوز المتبقّي | ✅ مرفوض بالسقف | ✅ |
| 51 | إرجاع بمفتاح مكرّر | ✅ مرفوض | ✅ |
| 52 | الاستثناءات ترصد الإلغاء والإرجاع | `order_voided` + `refund` بدرجة high | ✅ |
| 53 | فتح الدرج بلا بيع | حدث مسجَّل · **لا يغيّر النقد المتوقّع** | ✅ |
| 54 | تسليم درج معلّق ثانٍ لنفس الوردية | ✅ مرفوض (فهرس فريد جزئي) | ✅ |
| 55 | **مسؤولية الدرج لا تنتقل قبل التأكيد** | قبل: الباريستا · بعد: المُستلِم | ✅ |
| 56 | إعادة فتح تسليم مُنجَز | ✅ مرفوض | ✅ |
| 57 | سحب المبيعات مع إبقاء الفكّة | سُحب 6,000 · بقي 50,000 | ✅ |
| 58 | **الإغلاق الأعمى لا يُعيد رقماً** | `{"closed": true}` فقط، والأرقام في `audit_log` للمالك | ✅ |

#### ط) اليوم المحاسبي والمنتج الجديد (`db/tests/08` · `09` · `10`)

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 59 | وردية تُغلق ١ فجراً | كل فواتير الليلة على **اليوم الذي بدأت فيه** (٢٢٬٠٠٠ على ٦ سبتمبر) لا مقسومة على يومين | ✅ |
| 60 | فاتورة ٩ صباحاً في اليوم التالي | يوم محاسبي جديد | ✅ |
| 61 | منتج جديد بلا شراء | ✅ مرفوض — لا بيع من رصيد صفر | ✅ |
| 62 | منتج جديد بعد الشراء | يُباع، ويخصم وصفته من الدفتر | ✅ |

#### ي) محاكاة يومين: يوم أمين ويوم بسرقة (`db/tests/11`)

نفس حجم البيع في اليومين (١٩ طلباً · ٤٨٬٠٠٠). اليوم الأول باريستا أمين، والثاني
فيه ثلاث سرقات مختلفة الطبيعة. **المقياس مقياسان معاً: أن يُكشف كل شيء في اليوم
الثاني، وألّا يُتّهم أحد في الأول** — نظامٌ يصرخ كل يوم عديم الفائدة كنظام لا
يصرخ أبداً.

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 63 | **اليوم الأمين** (هدر معايرة ٣٦غ · مشروب موظف · عدّ مطابق · جرد مطابق) | **صفر تنبيهات** · فرق درج ٠ · فرق جرد ٠ | ✅ |
| 64 | (أ) ٣ مشروبات بلا فاتورة والمال في الجيب | لا يُرى في المبيعات ولا الدرج · يظهر **في الجرد** كنقص ٥٤غ + ٥٤٠مل | ✅ |
| 65 | **النقص مترجَم إلى مشروبات** | ٥٤غ = **٣ جرعات** رغم أنها ٠٫٤١٪ فقط · `over_threshold` (لا يختبئ داخل نسبة صغيرة) | ✅ |
| 66 | (ب) ٥٬٠٠٠ من الدرج | الإغلاق الأعمى يكشف فرقاً **−5,000** | ✅ |
| 67 | (ج) هدر وهمي ٢٠٠غ لتغطية (أ) | `high_waste`: ٢٣٦غ = **١٣ جرعة** في يوم واحد مقابل ٣٦غ أمس بنفس البيع | ✅ |
| 68 | **`high_waste` لا يُطلق على اليوم الأمين** | التنبيه مرّة واحدة، على اليوم الثاني وحده | ✅ |

#### ك) الرموز والموظفون (`db/tests/12`)

الرمز في هذا النظام **هويّة لا كلمة مرور**: هو ما يُثبت من دخل، ومن وافق على
إلغاء فاتورة، ومن أذن بإرجاع مال. وقد بقيت القاعدة الحيّة على الرمزين
الافتراضيين (المالك 1111 · الباريستا 0000) شهراً كاملاً — لأن `seed.sql` قال
«تُغيَّر فوراً من اللوحة» واللوحة لم تكن تحتوي تلك الشاشة. تعليقٌ لا يفرض شيئاً.

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 69 | الرمز لا يُقرأ | نفس الرمز يعطي تهشيرين مختلفين — لا استخراج ولا جدول عكسي | ✅ |
| 70 | التحقّق بالمقارنة لا بالقراءة | `pin_hash = crypt(pin, pin_hash)` | ✅ |
| 71 | تغيير الرمز | القديم يبطل · الجديد يعمل · `pin_changed_at` يُسجَّل | ✅ |
| 72 | **فراغ `pin_changed_at` = تحذير** | أحمر في لوحة المالك وفي شاشة الباريستا، لا يزول إلا بالتغيير | ✅ |
| 73 | الموظف يُعطَّل لا يُحذف | يخرج من شاشة الدخول، وفواتيره وورديّاته تبقى | ✅ |
| 74 | فكّ القفل بعد ٥ محاولات | `failed_pin_attempts = 0` و`locked_until = null` | ✅ |

ما تفرضه **طبقة التطبيق** لا القاعدة (`src/app/manage/users-actions.ts`) —
لأن القاعدة لا تخزّن إلا التهشير، وتمريرُ الرمز الصريح إليها يكتبه في سجلاتها:

- **رفض الرموز المكشوفة**: 0000 · 1111 … · 1234 · 4321 · وأي رقم مكرّر.
- **لا رمزان متشابهان** — وإلا صار «من وافق على الإلغاء؟» سؤالاً بلا جواب.
- **رمز المالك للموافقة** على إضافة موظف وتعيين رمز لغيره وتعطيله.
- **آخر مالك نشط لا يُعطَّل** — وإلا أُقفل النظام على الجميع.
- **من له وردية مفتوحة لا يُعطَّل** — وإلا بقي درج بلا مسؤول.
- **الباريستا يغيّر رمزه من شاشته** (`/pos`)، لأن `/manage` مقفلة عليه — ولو لم
  يكن الزرّ هناك لبقي على 0000 إلى الأبد. وهذا هو الخلل الأصلي بعينه.

#### ل) الربح (`db/tests/13`)

الإيراد ليس الربح. و«بعت ٤٨٬٠٠٠» لا يقول شيئاً عن حال المقهى — الذي يقوله هو
ما بقي بعد ثمن القهوة والحليب والكوب.

كان `inventory_transactions.unit_cost` يُملأ في `PURCHASE` وحده، وصفوف `SALE`
و`STAFF` و`WASTE` فارغةً. فأُضيف مُشغّل يختم التكلفة **لحظة الاستهلاك** (0024):
التكلفة واقعةٌ حدثت مرّة، ولو حُسبت لاحقاً من `materials.current_cost` لتبدّل
«ربح الشهر الماضي» كلّما غلا كيلو القهوة — وتقريرٌ يتغيّر بعد أن قُرئ لا يُبنى
عليه قرار.

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 75 | ختم التكلفة | كل حركة `SALE`/`STAFF`/`WASTE` تحمل تكلفة وحدتها تلقائياً | ✅ |
| 76 | تكلفة فاتورة لاتيه سفري | تطابق الوصفة بالدينار: ١٨غ حبوب + ١٨٠مل حليب + كوب + غطاء | ✅ |
| 77 | **غلاء القهوة لا يغيّر ربح الأمس** | التكلفة المرجّحة ٢٥ ← ٤٢، وتكلفة مبيع أمس لم تتحرّك | ✅ |
| 78 | الإرجاع يقضم الربح كلّه | الإيراد ينقص بمقدار المُرجَع، والمواد لا تعود | ✅ |
| 79 | الهدر مُقوَّم بتكلفة لحظته | ١٠٠غ × ٤٢ = ٤٬٢٠٠ لا × ٢٥ | ✅ |
| 80 | مشروب الموظف تكلفةٌ بلا إيراد | `cogs_free` منفصل، وخارج الإيراد | ✅ |
| 81 | **فاتورة بلا تكلفة: «—» لا «١٠٠٪»** | تُستثنى من الربح، ويُعرض عدد أكوابها | ✅ |
| 82 | التغطية مُعلَنة | «٧ من ٨ فاتورة» ظاهرة في الشاشة لا مخفيّة | ✅ |

الحالة ٨١ وُلدت من خلل حقيقي: أول تشغيل على القاعدة الحيّة أظهر **هامش ١٠٠٪**
لكل مشروب، لأن ٣١ بيعاً من قبل الختم ساهمت بتكلفة صفر فظهرت ربحاً صافياً.
الرقم الواثق الخاطئ أسوأ من الفراغ الصريح، فصار ما لا تكلفة له يُعرض «—»
ويُعلَن عدده. والحساب الدقيق يبدأ من يوم تشغيل الختم، ولا يُخترع لما قبله رقم.

#### م) النسخة الاحتياطية (`db/tests/backup-roundtrip.sh`)

نافذة الاسترجاع الزمني على Neon في الخطة الحالية **٦ ساعات** (`history_retention_seconds
= 21600`)، ولم تكن هناك أي نسخة محفوظة. فخطأٌ يحدث ليلاً ويُلاحَظ صباحاً يقع
خارج النافذة. ونسخةٌ تسكن مع أصلها ليست نسخة.

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 83 | استعادة على قاعدة **فارغة** (مخطّط وحده) | ٤٠ جدولاً تعود، وعدد صفوف كل جدول مطابق | ✅ |
| 84 | الطلبات | المعرّف والرقم والمبلغ والحالة مطابقة | ✅ |
| 85 | المخزون | `cached_stock` و`current_cost` مطابقان — لا يُعاد حسابهما | ✅ |
| 86 | الإعدادات `jsonb` | `"IQD"` تعود قيمةَ JSON لا نصّاً | ✅ |
| 87 | دفتر المخزون | مجموع `qty_delta` ومجموع التكاليف المختومة مطابقان | ✅ |
| 88 | **إيقاف المُشغّلات ضروري** | بلاه لا تكتمل الاستعادة أصلاً (مفتاح أجنبي) | ✅ |

الحالتان ٨٦ و٨٨ خللان حقيقيان كشفتهما أوّل استعادة، ولم يظهر أيٌّ منهما في
مراجعة الكود:

- `settings.value` من نوع `jsonb` يحمل `"IQD"`، والسوّاق يُعيده نصّاً عادياً لا
  يُفرَّق عن `text` — فكُتب `'IQD'` وهو ليس JSON صالحاً فرُفض. **نوع العمود لا
  يُستنتج من قيمته؛ يُقرأ من المخطّط.**
- بلا `session_replication_role = replica` تسقط الاستعادة على أول صفّ تدقيق
  يسبق `businesses`، ولو نجحت لأعاد مُشغّل الدفتر حساب `cached_stock` فوق
  المستعاد فتضاعف — فتعود القاعدة **مشابهةً** لا **مطابقة**، والنسخة التي
  تعود مشابهةً ليست نسخة.

#### ن) البيع بلا إنترنت (`db/tests/14`)

انقطاع الشبكة ليس حالةً نادرة، وبيعٌ لا يتمّ لأن الشبكة غابت خسارةٌ مضاعفة:
زبونٌ ينتظر ومالٌ لا يُسجَّل. فالكاشير يُتمّ البيع محلياً (`localStorage`)
ويرفعه حين تعود الشبكة.

والخطر هنا ليس فقدان الفاتورة — المفتاح الفريد يمنع تكرارها (§21) — بل
**وقتها**: بيعٌ تمّ ١١ ليلاً ورُفع ٨ صباحاً كان سيُكتب بوقت الرفع، فينتقل إلى
يوم محاسبي آخر ويظهر نقدُه في درج وردية لم تقبضه. كلّ ما بُني في 0017 يسقط.
فأُضيف `p_occurred_at` (0025)، وتُكتب اللحظة في الطلب والدفعة وحركة المخزون
وحركة الكاش **معاً** — لا في بعضها دون بعض، وإلا تناقض الدفتر مع الفاتورة.

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 89 | بيع مؤجّل يُرفع في اليوم التالي | يبقى في يومه المحاسبي (٢٠٢٦-٠٩-١٢) لا يوم الرفع | ✅ |
| 90 | اتّساق السجلّات | الفاتورة والدفعة والدفتر والدرج بلحظة واحدة | ✅ |
| 91 | رفع مكرّر لنفس المفتاح | `replay = true`، وعدد الفواتير لم يتغيّر | ✅ |
| 92 | نقد الليلة | في درج ورديتها هي، لا في المفتوحة وقت الرفع | ✅ |

**قرار بنيوي:** أُسقِطت نسخة `checkout` ذات ١١ وسيطاً. لأن الوسيط الثاني عشر
له قيمة افتراضية، فبقاء النسختين يجعل النداء بـ١١ وسيطاً **غامضاً**
(`function checkout(...) is not unique`) — أي أن كل بيع يتوقّف. كُشف هذا في
الاختبار قبل النشر.

**ما لا يفعله عامل الخدمة عمداً:** لا يخزّن `POST` ولا يُعيد إرساله. البيع
يمرّ من الطابور الذي يعرف المفتاح الفريد ووقت البيع — وهما ما يجعل الرفع
لاحقاً صحيحاً. وعاملُ خدمةٍ «يُعيد إرسال» طلباً لا يعرف محتواه يُنتج فواتير
مكرّرة.

**ثلاث قواعد في الطابور:** لا يُحذف بيع إلا بتأكيد الخادم · المفتاح يُولَّد
مرّة ويبقى · الوقت يُلتقط لحظة البيع. وإذا رفض الخادم فاتورةً ثلاث مرّات
تُعرض للباريستا بسببها ولا تُبتلع: مالٌ في الدرج بلا فاتورة سيظهر فرقاً عند
العدّ.

#### س) الصلاحيات (`db/tests/15`)

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 93 | الباريستا يقرأ وردية مفتوحة | لا يرى المتوقّع في الدرج — العدّ يبقى أعمى | ✅ |
| 94 | صلاحيات المال والتقارير والإدارة | ليست بيده ولا واحدة منها | ✅ |
| 95 | ما يملكه فعلاً | بيع · هدر · فتح/إغلاق وردية — لا أكثر | ✅ |

#### ع) من يعدّ الدرج (`db/tests/16`)

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 96 | عدّ وردية عُدّت مرّة | مرفوض — العدّ لا يُكرَّر فيُستبدل الفرق | ✅ |
| 97 | عدّ وردية أُغلقت بوضع «لا أحد» | مرفوض | ✅ |
| 98 | عدّ المالك صباحاً | الفرق نفسه الذي كان سينتجه عدّ الباريستا ليلاً | ✅ |
| 99 | وردية أُنهيت بلا عدّ | المعدود **فارغ**، لا صفرٌ يُقرأ «طابق» | ✅ |
| 100 | تنبيهات الفروقات | لا فرق كاذب على وردية لم تُعدّ بعد | ✅ |
| 101 | قائمة «بانتظار العدّ» | تخلو ممّا عُدّ | ✅ |
| 102 | تغيير السياسة لاحقاً | لا يُحيي ورديات أُغلقت تحت سياسة أخرى | ✅ |

**قرار بنيوي:** السياسة تُحفظ في `shifts.count_mode` لحظة الإغلاق لا تُقرأ من
`branches` وقت العرض. وإلا لظهرت ورديات الشهر الماضي فجأةً «بانتظار العدّ»
لأن المالك بدّل الإعداد اليوم.

#### ف) المخزون المتقدّم (`db/tests/17`)

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 103 | شراء «٢ كرتون» (١٢٬٠٠٠ مل للكرتون) | الدفتر ٢٤٬٠٠٠ مل، والوحدة محفوظة **مع** الصفّ | ✅ |
| 104 | تكلفة ١٢٬٠٠٠ للكرتون | ١ دينار للمل — التحويل في القاعدة لا في الرأس | ✅ |
| 105 | قائمة الشراء | الناقص بوحدة الشراء مُقرَّباً **لأعلى** (نصف كرتون لا يُشترى) | ✅ |
| 106 | جرد مادّتين من تسع | مُعلَّم `is_partial` — لا يُقرأ جرداً شاملاً | ✅ |
| 107 | هدر بأسباب مختلفة | موزّعٌ على أسبابه **بالدينار** لا بالغرام | ✅ |

**قرار بنيوي:** الوحدة تُكتب مع إدراج الصفّ لا بتحديثٍ بعده — الدفتر للإلحاق
فقط (0011) فرفض الحارس التحديث، وهو محقّ. ولهذا صار في `record_purchase`
وسيطان اختياريان، و**أُسقِط توقيعها بسبعة وسائط** لئلا يصير النداء غامضاً
كما حدث مع `checkout` في 0025.

#### ص) الدوام والوقت الإضافي (`db/tests/18`)

الدوام الرسمي ٤ عصراً ← ١٢ ليلاً (على الفرع، يغيّره المالك). وما بعده إضافيّ،
ويُحتسب أجراً **إن رافقته فاتورة**: مالٌ دخل الدرج في تلك الدقيقة دليلٌ أصعب
على التزوير من بصمة حضور.

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 108 | وردية ٤ عصراً ← ١١:٣٠ ليلاً | ٤٥٠ دقيقة عاديّ · صفر إضافيّ | ✅ |
| 109 | وردية امتدّت لـ٢ فجراً وباعت بعد منتصف الليل | ٤٨٠ عاديّ · ١٢٠ إضافيّ · فاتورته معه | ✅ |
| 110 | وردية بدأت ٣:٣٠ وامتدّت لـ١ فجراً بلا بيع | ٣٠ د قبل الدوام تُعرض · ٦٠ د بعده بلا فاتورة | ✅ |
| 111 | كشف المدّة | يُدفع ١٢٠ د (أُثبتت) ولا يُدفع ٦٠ د (لم تُثبَت) | ✅ |
| 112 | البيع بلا إنترنت في اللوحة | عدد الفواتير ووقت آخر مزامنة يظهران للمالك | ✅ |

**قراران بنيويان.** الأول: الدقائق التي لا تُثبتها فواتير **تُعرض ولا تُخفى**.
نظامٌ يحذفها يظلم من عمل ولم يبع، ونظامٌ يجمعها بلا تمييز يجعل المالك يدفع
على البقاء. فالثلاثة أعمدة تُعرض والقرار له.

الثاني: `orders.synced_at` تُختم **داخل** `checkout` لا بتحديثٍ بعده — الدفتر
للإلحاق فقط. وقبلها كانت الإدارة تعرف أن البيع بلا إنترنت يعمل، ولا تعرف
أنه حدث: طابور المتصفّح لا يراه إلا صاحب الجهاز.

#### ق) نسخ الوصفة وربط المحاصيل (`db/tests/19`)

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 113 | تعديل الحليب ١٨٠ ← ٢٠٠ | نسخة جديدة، والسابقة معطَّلة لا ممحوّة | ✅ |
| 114 | قراءة النسخة القديمة | باقية بمكوّناتها — يُقرأ منها تاريخ التغيير | ✅ |
| 115 | حفظٌ بلا تغيير (بترتيبٍ مختلف) | لا نسخة جديدة — التاريخ لا يُغرَق بنسخٍ فارغة | ✅ |
| 116 | أثر التدقيق | صفٌّ واحد بقبله وبعده | ✅ |
| 117 | «كالدي: إسبريسو وتقطير فقط» | لا يظهر على اللاتيه | ✅ |
| 118 | إيقاف محصول عن مشروب | يختفي من البيع ويبقى تاريخه مقروءاً | ✅ |

**قرار بنيوي:** العملية كلّها في دالّة قاعدة واحدة (`set_recipe`) لا في عدّة
جُمل من الخادم، لأن `uq_recipe_active` يمنع وصفتين فعّالتين — فانقطاعٌ بين
«عطّل القديمة» و«أدرِج الجديدة» يترك المشروب **بلا وصفة**، ومشروبٌ بلا وصفة
لا يُباع.

#### ر) تصفير بيانات التجربة (`db/tests/20`)

بيانات التجربة ليست محايدة: تدخل في الأرباح وفي متوسّط البيع وفي معدّل
الاستهلاك، فيقرأ المالك أرقاماً مخلوطةً بلعب. والحذف يدوياً من خارج التطبيق
لا يُسجَّل ولا يُحاسَب عليه — فصار **زرّاً** بثلاثة حرّاس.

| # | الحالة | المتوقّع | النتيجة |
|---|---|---|:--:|
| 119 | تصفير بلا كلمة التأكيد | مرفوض — ولا صفّ يُحذف | ✅ |
| 120 | تصفير بيد الباريستا | مرفوض — الدور يُفحص في القاعدة لا في الواجهة | ✅ |
| 121 | تصفير المالك | الحركات والمخزون صفر، والرصيد يتبع دفتره | ✅ |
| 122 | الكتالوج والمستخدمون | لم يُمسّوا — مشروبات ومواد ووصفات ومستخدمون كما كانوا | ✅ |
| 123 | بعد التصفير | أوّل فاتورة ١٠٠١، وسجلّ تدقيقٍ صفُّه الأول يشرح التصفير | ✅ |

**قرار بنيوي:** `truncate` هو المخرج الوحيد الذي لا يكسر حارس «للإلحاق فقط»
ولا يلتفّ عليه — فهو لا يُشغّل مُشغّلات الصفوف أصلاً. وثمنه أنه لا يعرف
`business_id`، فالدالّة **ترفض العمل** على قاعدةٍ فيها أكثر من عمل بدل أن
تمحو عمل غيره. والحارس على `materials` يُوقَف سطراً واحداً ويُعاد في السطر
الذي يليه، والاختبار يثبت عودته.

#### ز) اختبارات الوحدة (`npm test`)

22 اختباراً تمرّ: تنسيق الدينار بأرقام إنجليزية بلا كسور · تحويل الوحدات
(كغ↔غ، لتر↔مل) · التكلفة لكل وحدة أساس · **توحيد رقم الهاتف العراقي** (كل صيغ
الرقم الواحد تعطي مفتاحاً واحداً — وإلا انقسم حساب الولاء) · **صياغة «كم
مشروباً»** بجمع عربي سليم (مشروب واحد · مشروبين · ٣ مشروبات · ١٣ مشروباً).
و`npm run typecheck` و`npm run build` يمرّان.

تشغيل اختبارات القاعدة كلّها دفعةً واحدة: `db/tests/run-all.sh` — يُصفّر بين كل
سكربت والذي يليه، لأن كلّاً منها يفتح وردية ولا يُسمح بأكثر من وردية مفتوحة.
...
| 2. الدخول والصلاحيات | ✅ — كل الكود انتقل للمفاتيح المنقّطة |
| 3. المخزون: مواد · وصفات · دفتر · جرد · هدر · مشروب موظف | ✅ |
| 4. البيع: منتجات · طلبات · بنود · دفع · معاملة ذرّية | ✅ |
| 5. الكاش: ورديات · فكّة · سحب · عدّ أعمى · تسليم | ✅ قاعدةً وواجهةً |
| 6. الولاء: تسجيل · OTP · حساب · دفتر · مكافآت · صرف | ✅ — ينقصه **مزوّد رسائل الواتساب** فقط |
| 7. التقارير | ✅ دوالّ · ⏳ شاشات أعمق (أداء كل باريستا) |
| 8. التدقيق | ✅ |
| 9. كشف الاستثناءات | ✅ قاعدةً وشاشةً |
| 10. تحسين الواجهة | ⏳ |
